export default {
    productAdd: async (_, { data }, { model: { product } }) => {
        const newProduct = await product.create(data);
        if (!newProduct) throw new Error('Error for adding product');
        return newProduct;
    },
    productUpdate: async (_, { id, data }, { model: { product } }) => {
    ...
    productRemove: async (_, { id }, { model: { product } }) => {
        console.log(`ID = ${id}`);
    ...
    productRemoveAll: async (_, args, { model: { product } }) => {
